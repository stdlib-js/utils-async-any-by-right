...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '1Qp2V,FD', options ) );
	}
	...
	if ( hasOwnProp( options, 'series' ) ) {
		...
		if ( !isBoolean( opts.series ) ) {
			return new TypeError( format( '1Qp2o,GE', 'series', opts.series ) );
		}
	}
	if ( hasOwnProp( options, 'limit' ) ) {
		opts.limit = options.limit;
		if ( !isPositiveInteger( opts.limit ) ) {
			return new TypeError( format( '1Qp3P,Fv', 'limit', opts.limit ) );
		}
	}
	return null;
}
